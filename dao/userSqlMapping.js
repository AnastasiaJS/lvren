...
var card = {
    ...
    getOrder1:'SELECT date_format(OrderTime," %Y-%c-%d %H:%i") OrderTime,State,Oid,Tid,Anhao,Wechat,`Name`,HeadPic,`user`.Uid,Appointment FROM `order`,`user` WHERE `order`.Uid=`user`.Uid AND `order`.Tid=? AND State LIKE ?',
};
module.exports = card;
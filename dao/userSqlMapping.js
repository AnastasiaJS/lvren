...
var card = {
    ...
    addCard: 'INSERT INTO tourcard(Uid,Title,About,Addr,Price,CanCut,Play,Other,AppointTime,AboutPrice,Face,Photos,Sort) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
    updateCard: 'UPDATE tourcard SET Title=?,About=?,Addr=?,Price=?,CanCut=?,Play=?,Other=?,AppointTime=?,AboutPrice=?,Face=?,Photos=?,Sort=? WHERE Uid=?',
    ...
    getOrder0: 'SELECT date_format(OrderTime,"%Y-%c-%d %H:%i") OrderTime,State,Oid,`order`.Price Price,Tid,Addr,Anhao,Wechat,`Name`,tourcard.Uid Uid,Face,Title,Addr,Appointment FROM `order`,tourcard,`user` ' +
    'WHERE `order`.Tid=tourcard.Uid AND tourcard.Uid=`user`.Uid AND `order`.Uid=? AND State LIKE ? ' +
    'ORDER BY Oid DESC',//我在租谁
    getOrder1: 'SELECT date_format(OrderTime,"%Y-%c-%d %H:%i") OrderTime,State,Oid,`order`.Price Price,Tid,Addr,Anhao,Wechat,`Name`,HeadPic,`user`.Uid,Appointment ' +
    'FROM `order`,`user`,tourcard ' +
    'WHERE `order`.Uid=`user`.Uid AND tourcard.Uid=`order`.Tid AND `order`.Tid=? AND State LIKE ? ' +
    'ORDER BY Oid DESC',//谁在租我
    ...
    getReplies: 'SELECT *,date_format(reply.Rtime,"%Y-%c-%d %H:%i") Time FROM reply,tourcard,`user`,message WHERE tourcard.Uid=? AND tourcard.Uid=message.Tid AND message.Mid=reply.Mid and reply.Uid=`user`.Uid',//查找所有留言
    getarticles: 'SELECT * FROM articles',
    getNewsNum1: 'SELECT COUNT(news.Nid) num FROM news WHERE Tid=?  AND Type=2 AND Flag=0',
    getNewsNum2: 'SELECT COUNT(news.Nid) num FROM news WHERE Tid=?  AND Type=0 AND Flag=0',
    getNewsNum3: 'SELECT COUNT(news.Nid) num FROM news,message,(SELECT DISTINCT *,CONCAT("R",reply.Rid) id FROM reply) r WHERE news.Nid =r.id AND r.mid=message.Mid and message.Uid=? AND Type=1 AND Flag=0',
};
module.exports = card;